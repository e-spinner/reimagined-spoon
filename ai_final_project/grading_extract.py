...
def _collect_numeric_candidates(crop_gray: np.ndarray) -> list[CandidateNumber]:
    ...
    # EasyOCR + docTR only (no system Tesseract). Run both each variant so misreads from
    # one engine can be corrected by the other.
    ...
    for variant_name, image in variants:
        for engine, task in engines:
            ...
            # Do not stop after the first decimal: a fast engine can misread (e.g. 3.45 as 3.15)
            # while a later engine on the same variant still needs to run for a correct tie-break.
