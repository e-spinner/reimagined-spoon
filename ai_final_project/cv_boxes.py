...
# Must match ``dpi`` passed to ``detect_answer_region_from_pdf`` when converting
# pixel rectangles back onto the PDF (e.g. overlays).
CV_DETECTION_DPI = 220


def answer_box_rect_to_pdf_rect(rect: Rect, dpi: int = CV_DETECTION_DPI) -> fitz.Rect:
    """Map OpenCV box coordinates from a ``get_pixmap`` render into PDF points."""
    s = 72.0 / float(dpi)
    return fitz.Rect(rect.x * s, rect.y * s, (rect.x + rect.w) * s, (rect.y + rect.h) * s)
